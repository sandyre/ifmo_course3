...
void run_distr_app(DApp * app)
{
    ...
    if(app->nProcessID == 0) // parent code
    {
        ...
        while(histories_remaining)
        {
                ...
                history_msg.s_header.s_type == BALANCE_HISTORY)
            {
                ...
                lamp_local_time = max_timestamp(lamp_local_time, history_msg.s_header.s_local_time);
                ++lamp_local_time;
            }
            else
            {
    ...
    else // child code
    {
        ...
        timestamp_t last_history_row = get_lamport_time();
            ...
            for(int i = 1; i < app->nProcessesCount; ++i)
            {
                ...
                if(receive(app, i, &rmsg) == 0)
                {
                    if(rmsg.s_header.s_type == STARTED)
                    {
                        ...
                        lamp_local_time = max_timestamp(lamp_local_time, rmsg.s_header.s_local_time);
                        ++lamp_local_time;
                    }
                    else
                    {
            ...
            if(receive_any(app, &rmsg) == 0)
            {
                ...
                if(rmsg.s_header.s_type == TRANSFER)
                {
                    ...
                    if(order->s_src == app->nProcessID)
                    {
                        ...
                        last_history_row = get_lamport_time();
                        ...
                        history.s_history[cur_time + 2].s_balance = app->nBalance;
                        ...
                        ++lamp_local_time;
                        rmsg.s_header.s_local_time = get_lamport_time();
