...
int send(void * self, local_id dst, const Message * msg)
{
    ...
    if(write(app->ppChannels[app->nProcessID][dst].wf,
             msg,
             msg->s_header.s_payload_len + sizeof(MessageHeader)) != (msg->s_header.s_payload_len + sizeof(MessageHeader)))
    {
    ...
    while(replies_receiving)
    {
        ...
        else if(rec_msg.s_header.s_type == CS_REQUEST)
        {
            if((request_time > rec_msg.s_header.s_local_time) ||
                ((request_time == rec_msg.s_header.s_local_time) && (app->nProcessID > sender_id)))
            {
                Message rep_msg;
                ...
                send(app, sender_id, &rep_msg);
            }
            else
            {
                app->paDRs[sender_id] = 1;
            }
        }
...
void run_distr_app(DApp * app)
{
    ...
    app->paDRs = (char*)malloc(app->nProcessesCount);
    memset(app->paDRs, 0, app->nProcessesCount * sizeof(char));
